...
c = lines(8);    
    for i = 1:skip:numPts
        ...
        % trail
        nStart = max(1, i-trailSize);
        plot( v(nStart:i,:) , w(nStart:i,:), 'linewidth',3);  hold on;
        % current position
        plot( v(i,:) , w(i,:), 'k.', 'linewidth',2,'MarkerSize',20); hold on;           
        ...
        drawnow;
        if ( i == 1)
            disp('Set screen capture device to record, then press key to continue');
           pause; 
        end        
    end
